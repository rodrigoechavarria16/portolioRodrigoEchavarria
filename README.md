...
I am a Mechatronics Engineer graduated from Tecnológico de Monterrey (CCM campus), specializing in the design and implementation of intelligent systems at the intersection of hardware, software, automation, and cybersecurity. Currently working as a **Cybersecurity Advocate at Honeywell HTCS** (since August 2025), where I ensure Secure Software Development Lifecycle (SSDLC) compliance across product releases and contribute to software development initiatives through Sanity Testing and Regression Testing on iOS and Windows environments.
...
└── MediaFiles/                 ← All images and videos used in the portfolio
    ...
    ├── SDEToolDevelopment.jpg       → Experience: Honeywell — SDE Tool
    ├── DashboardDigitaltwin.png     → Experience: Siemens Digital Twin Dashboard
    ├── IsometricViewPlant.png       → Experience: Siemens Plant Isometric View
    ├── HMI_Digitaltwin.png          → Experience & Projects: Ford / Siemens HMI
    ├── DashboardInsightsHub.jpg     → Project: Cyber-Physical Warehouse (Insights Hub)
    ├── IOT_Communication.png        → Project: Cyber-Physical Warehouse (Node-Red)
    ├── VIN_Analysis.png             → Project: Autonomous Manufacturing DT (VIN)
    ├── MainHMI.png                  → Project: Autonomous Manufacturing DT (HMI)
    ...
    ├── SAIA.jpg                     → Project: SAIA aeronautical student society
    └── SAIA_Airplane.mp4            → Project: SAIA airplane design (demo video)
```
...
| **Dark / Light mode** | Toggle button (🌙/☀️) in the nav bar. Preference saved in `localStorage`. Respects OS setting on first visit |
| **3D skill sphere** | Interactive Three.js globe — drag, scroll to zoom, hover to inspect, click to filter by category |
| **Experience pivot tabs** | Microsoft-style tab strip: filter between All roles · Industry · Academic |
| **Coming Soon cards** | In-progress experiences and projects shown with animated "In progress" badges and dashed styling |
...
### Honeywell HTCS — Cybersecurity Advocate *(August 2025 – Present)*
Responsible for ensuring Secure Software Development Lifecycle (SSDLC) compliance across product releases while actively contributing to Software Development initiatives. Manages cross-platform quality assurance through Sanity Testing and comprehensive Regression Tests (Views and Functions) for iOS and Windows environments.

### Siemens Digital Industries Software — Digital Twin Integration Engineer *(Academic)*
Designed and developed a Cyber-Physical Warehouse System featuring a Digital Twin synchronized via Node-RED and TIA Portal. Integrated a ROS2-based AGV for autonomous navigation and utilized a multi-objective genetic algorithm for energy-efficient path planning. Built a full-stack IoT architecture streaming hardware telemetry to AWS serverless services and Siemens Insights Hub.

### Ford Motor Company — Digital Twin Developer *(Academic)*
Engineered a comprehensive improvement plan for vehicle final assembly using Virtual Commissioning and Digital Twin modeling. Integrated Vision-based VIN verification (OCR) and Pick-to-Light systems. Validated all logic through Emulate3D, Studio 5000, and FactoryTalkView, ensuring 100% integration readiness.

### Siemens DISW — OEE Monitoring & Digital Twin Simulation *(In progress · Bachelor's Final Project)*
Integrates a digital twin and real-time OEE monitoring using Siemens DISW tools — Plant Simulation, Insights Hub, and Mendix — to simulate, analyze, and optimize manufacturing processes.
...
| Cyber-Physical Warehouse + Digital Twin | Siemens · Digital Twin | `DashboardInsightsHub.jpg`, `NodeRedConnections.png` |
| Autonomous Manufacturing Digital Twin | Ford · Manufacturing | `VIN_Analysis.png`, `MainHMI.png` |
...
| Aeronautics & Electric Vehicle Systems | Team · SAIA & Electratón CCM | `SAIA.jpg`, `SAIA_Airplane.mp4` |
| OEE Monitoring & Digital Twin Simulation | Siemens · Smart Manufacturing | *(coming soon)* |
...
### Cybersecurity
`SSDLC` · `PSJira` · `SD Elements` · `Black Duck` · `Threat Modeling`
...
The **🌙 / ☀️ toggle** is in the top-right corner of the nav bar.
